...
export function AddRecipe({ parentState, updateParentState, close, onSaveCallback  }: AddRecipeProps) {
    ...
    const { state, updateState } = useCustomState<AddRecipeState>({
        isAddPantry: false,
        roastDate: '',
        expiryDate: '',
    });
    ...
    const onSave = () => {
        const userRecipe: UserRecipe = {
            ...
            roasterId: '',
            roasterName: parentState.roaster ?? 'unknown',
            origins: '',
            tastingNotes: parentState.tastingNotes?? 'unknown',
            dose: parentState.dose?? 'unknown',
            yield: parentState.yield?? 'unknown',
            time: parentState.time?? 'unknown',
            rating: parentState.rating?? 0,
            buyLink: ''
        };
